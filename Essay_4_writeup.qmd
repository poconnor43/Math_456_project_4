...
We will begin by visualizing potential outliers in our dataset using boxplots, grouping variables into two sets to efficiently display them. Next, we will apply an outlier removal function based on the **Interquartile Range (IQR) method**, identifying and filtering values that fall beyond 1.5 times the IQR to maintain statistical integrity. This function will be systematically applied to key variables, including **fixed acidity, volatile acidity, citric acid, residual sugar, chlorides, free sulfur dioxide, total sulfur dioxide, density, pH, sulphates, and alcohol**, ensuring that extreme values do not distort analysis. Finally, we will summarize the cleaned dataset and determine its new size, confirming that our filtering process retains a robust and reliable dataset for further analysis.
...
units <- c(
  ...
  "total.sulfur.dioxide" = "mg/L", "density" = "g/cm³", "pH" = "", 
  "sulphates" = "g/L", "alcohol" = "%"
)

# Boxplots to detect outliers with units in labels
par(mfrow = c(2, 3))
for (i in 1:6) {
  boxplot(data[[i]], main = names(data)[i], ylab = paste0("(", units[names(data)[i]], ")"))
}

# Boxplots for the remaining variables
par(mfrow = c(2, 3))
for (i in 7:12) {
  boxplot(data[[i]], main = names(data)[i], ylab = paste0("(", units[names(data)[i]], ")"))
}
